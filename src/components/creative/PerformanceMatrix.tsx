      ...
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 20, bottom: 10, left: 0 }}>
            <XAxis type="number" dataKey="x" name="Creative Score" domain={[1, 5]} tick={{ fontSize: 11, fill: "#6B7484" }} label={{ value: "Creative Score", position: "insideBottom", offset: -5, fontSize: 11, fill: "#6B7484" }} />
            <YAxis type="number" dataKey="y" name="Performance" domain={[0, 100]} tick={{ fontSize: 11, fill: "#6B7484" }} label={{ value: "Performance", angle: -90, position: "insideLeft", fontSize: 11, fill: "#6B7484" }} />
            <ZAxis range={[80, 80]} />
            ...
            <Tooltip
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <div className="bg-white border border-navy-900/10 rounded-lg px-3 py-2 text-xs shadow-lift">
            ...
            <Scatter
              data={data}
              shape={(props: any) => (
                <circle cx={props.cx} cy={props.cy} r={5} fill={PLATFORM_COLORS[props.payload.platform] ?? "#2FBF9F"} fillOpacity={0.85} />
              )}
            />
          </ScatterChart>
        </ResponsiveContainer>
