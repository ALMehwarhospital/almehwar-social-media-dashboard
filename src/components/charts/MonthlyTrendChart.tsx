...
export function MonthlyTrendChart({scope='total',untilMonth}:{scope?:'total'|'organic'|'paid';untilMonth?:string}){
  ...
  return <div>
    ...
    <div className='h-64'><ResponsiveContainer width='100%' height='100%'><AreaChart data={chartData}><defs><linearGradient id='trendFill' x1='0' y1='0' x2='0' y2='1'><stop offset='0%' stopColor='#2FBF9F' stopOpacity={0.35}/><stop offset='100%' stopColor='#2FBF9F' stopOpacity={0.02}/></linearGradient></defs><CartesianGrid strokeDasharray='3 3' stroke='#E2EAEF' vertical={false}/><XAxis dataKey='label' tick={{fontSize:12,fill:'#7D6D59'}} axisLine={false} tickLine={false}/><YAxis tick={{fontSize:11,fill:'#7D6D59'}} axisLine={false} tickLine={false} tickFormatter={v=>formatNumber(v)} width={52}/><Tooltip formatter={v=>[formatNumber(Number(v)),METRICS.find(m=>m.key===metric)?.label??'']} contentStyle={{borderRadius:12,border:'1px solid #E2EAEF',fontSize:13,background:'#FFFFFF',color:'#0E3145'}}/><Area type='monotone' dataKey='value' stroke='#2FBF9F' strokeWidth={2.5} fill='url(#trendFill)'/></AreaChart></ResponsiveContainer></div>
  </div>
}
