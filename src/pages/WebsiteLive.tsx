...
const n = (v: unknown) => {
  ...
  return value === null ? "N/A" : new Intl.NumberFormat("en").format(Math.round(value));
};
const pct = (v: unknown, d = 1) => {
  const value = finite(v);
  return value === null ? "N/A" : `${(value * 100).toFixed(d)}%`;
};
const decimal = (v: unknown, d = 2) => {
  const value = finite(v);
  return value === null ? "N/A" : value.toFixed(d);
};
const short = (v: unknown) => {
  const value = finite(v);
  return value === null ? "N/A" : new Intl.NumberFormat("en", { notation: "compact", maximumFractionDigits: 1 }).format(value);
};
