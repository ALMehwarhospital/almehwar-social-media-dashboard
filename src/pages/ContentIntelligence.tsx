...
function metricValue(item: any, key: string): number | null {
...
export default function ContentIntelligence() {
  ...
  const rankable = items.filter((item: any) => metricValue(item, rank) !== null);
  const excludedCount = items.length - rankable.length;

  const sorted = [...rankable].sort((a, b) => {
    const av = metricValue(a, rank) ?? -Infinity;
    const bv = metricValue(b, rank) ?? -Infinity;
    return bv - av;
  });

  const peerRatio = (item: any): number | null => {
    const value = metricValue(item, rank);
    ...
    const peers = items.filter(
      (p: any) =>
        ...
        metricValue(p, rank) !== null
    );

    const vals = peers
      .map((p: any) => metricValue(p, rank))
      .filter((v): v is number => v !== null);

    if (!vals.length) return null;
    const avg = vals.reduce((a, b) => a + b, 0) / vals.length;

    if (avg === 0) return value === 0 ? 1 : null;
    return value / avg;
  };

  const underperformers = rankable
    .map((item: any) => ({ item, ratio: peerRatio(item) }))
    .filter((row): row is { item: any; ratio: number } => row.ratio !== null)
    ...
    <div className="space-y-10">
      ...
      {isLiveMonth && (
        <div className="rounded-xl border border-mint-300/30 bg-mint-100/60 p-3 text-xs text-navy-700">
          {live.isLive ? "LIVE MTD" : "SNAPSHOT MTD"}: {items.length} content items loaded for {month}.
          {pendingClassification > 0 ? ` ${pendingClassification} current-month items are awaiting Content Pillar classification, so they remain under Other until reviewed.` : ""}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <p className="text-[11px] text-fog-500 mb-1">Highest reported result for the selected metric.</p>
          <p className="text-[10px] text-fog-400 mb-3">{rankable.length}/{items.length} items have this metric{excludedCount ? ` · ${excludedCount} N/A excluded` : ""}.</p>
          {sorted.length ? <ContentTable items={sorted.slice(0, 5)} metric={rank as any} /> : <EmptyState message="No items have this metric available." />}
        </Card>

        <Card>
          <p className="text-xs uppercase tracking-wide text-signal-coral font-semibold mb-1">Contextual Underperformers</p>
          <p className="text-[11px] text-fog-500 mb-1">Lowest result relative to the same platform, spend type and format.</p>
          <p className="text-[10px] text-fog-400 mb-3">Only items with a real selected metric and a valid peer comparison are eligible.</p>
          {underperformers.length ? <ContentTable items={underperformers.slice(0, 5)} metric={rank as any} /> : <EmptyState message="Not enough comparable data for this metric." />}
        </Card>
      </section>
      ...
      <section>
        <SectionHeader
          eyebrow="Pillars"
          title="Content Pillar Analysis"
          description={pendingClassification ? "Current-month pillar classification is still pending for newly synced content." : "Uses the same active filters as the content table."}
        />
          ...
          {pillars.slice(0, 6).map((p) => (
            <Card key={p.pillar}>
              ...
              <div className="mt-4 text-sm space-y-1">
                ...
                <p>Avg Engagement <b>{formatRate(p.avgEngagement)}</b></p>
      ...
      <section>
        <SectionHeader
          ...
          description="Average engagement uses only content where a valid platform-specific engagement rate is available."
        />
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {formats.map((f) => (
            <Card key={f.format}>
              <p className="font-medium">{f.format}</p>
              <p className="font-display text-2xl mt-2">{formatRate(f.avgEngagement)}</p>
