...
import { monthLabel, formatNumber, pctChange } from "../utils/format";
...
function PlatformCompareCard({platform,series,accent}:{platform:Platform;series:ReturnType<typeof getPlatformSeries>;accent:"mint"|"blue"}){const latest=series[series.length-1];const prev=series[series.length-2];if(!latest)return null;const change=prev?pctChange(latest.reach,prev.reach):0;const maxReach=Math.max(1,...series.map((s)=>s?.reach??0));return <Card><div className="flex items-center justify-between mb-4"><h3 className="font-display text-lg text-navy-900">{platform}</h3>{prev&&<TrendTag direction={change>=0?"up":"down"} value={change}/>}</div><div className="space-y-2.5">{series.map((row,i)=>row&&<div key={i} className="flex items-center gap-3"><span className="text-xs text-fog-500 w-10 shrink-0">{monthLabel(row.month)}</span><div className="flex-1 h-2 rounded-full bg-warm-100 overflow-hidden"><div className={`h-full rounded-full ${accent==="mint"?"bg-mint-500":"bg-signal-blue"}`} style={{width:`${Math.min(100,(row.reach/maxReach)*100)}%`}}/></div><span className="text-xs font-mono text-navy-700 w-16 text-right">{formatNumber(row.reach)}</span></div>)}</div><p className="text-fog-500 text-xs mt-3 pt-3 border-t border-navy-900/6">{latest.engagementRate}% engagement using {latest.engagementDenominator} · +{latest.followersGrowth} followers this month</p></Card>}
